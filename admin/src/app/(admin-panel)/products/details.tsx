  ...
  useEffect(() => {
      ...
      inventories:
        product.inventoryRef?.length
          ? product.inventoryRef.map((item: any) => ({
            ...
            ...(item.mrpPrice && { mrpPrice: item.mrpPrice }),
          }))
          : [{ quantity: String(product.mainInventory) }],
    ...
    replaceFabric(
      product.fabrics?.length
        ? product.fabrics
        : [{ colorCode: "#1677ff", colorName: "", images: [] }]
    );
