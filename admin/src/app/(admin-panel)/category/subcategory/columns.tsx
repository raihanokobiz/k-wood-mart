      ...
      cell: ({ row }) => {
        ...
        const handleToggle = async (checked: boolean) => {
          try {
            ...
            const activeCount = tableData.filter(item => item._id !== sub._id && item.isSpecial).length;

            console.log(activeCount, "activeCount");
            

            if (checked && activeCount >= 3 && !sub.isSpecial) {
              alert("Maximum 3 special sub-categories allowed!");
              return;
            }
