...
class ProductService extends BaseService {
  ...
  async getNewArrivalsProduct() {
    try {
      const newArrivals = await this.#repository.findAll(
        { isNewArrival: true },
        ["categoryRef"], 
        8 
      );
  ...
  async updateProductStatusisNewArrival(id, status) {
    if (!status) throw new NotFoundError("Status is required");
    const booleanStatus = status === "true" || status === true;

    const product = await this.#repository.updateProductStatusisNewArrival(id, {
      isNewArrival: booleanStatus,
    });
